...
const getUsersByGender = (users, gender) => {
  const filtred = users.filter(user => gender === user.gender);
...
const getUserByEmail = (users, email) => {
  const filtred = users.find(user => email === user.email);
  return filtred;
};
...
const getUsersWithAge = (users, min, max) => {
  const filtred = users.filter(user => (min < user.age) & (max > user.age));
  return filtred;
};
...
const getTotalBalance = users => {
  const balanceSum = users.reduce(
    (totalSum, user) => totalSum + user.balance,
    0,
  );
  return balanceSum;
};
...
const getUsersByFriend = (users, name) => {
  const filtred = users.filter(user => user.friends.includes(name));
  return filtred.map(user => user.name);
};
